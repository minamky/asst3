...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double overallKernalDuration = kernalEndTime - kernalStartTime;
    printf("Kernal Effective BW by CUDA saxpy: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * overallKernalDuration, GBPerSec(totalBytes, overallKernalDuration));
